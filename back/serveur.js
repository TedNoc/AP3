...
app.get('/Bdd', async(req,res) => { // définition d'une route pour récupérer les données de la base de données
    ...
    try{
        ...
        const rows = await conn.query('SELECT * FROM utilisateur'); // exécution d'une requête pour récupérer toutes les données de la table ap2
...
app.post('/inscr', async (req, res) => {
  ...
  .then (async(hash) => {
      ...
      // On exécute une requête INSERT pour insérer une nouvelle ligne dans la table 'maison'
      

      let requete = 'INSERT INTO utilisateur (mail, mdp) VALUES (?, ?);'
      let rows = await conn.query(requete, [req.body.mail,hash]);
...
app.post('/connexion', async (req, res) => { 
 ...
 const rows = await conn.query('SELECT * FROM utilisateur WHERE mail = ?', [mail]);
